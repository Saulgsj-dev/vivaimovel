  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let imagem of dados.imagens) {
        // Nome seguro (sem acentos, espaços ou caracteres especiais)
        const cleanName = imagem.name.toLowerCase().replace(/[^a-z0-9._-]/g, "_");
        const filePath = `${usuario.id}/${uuid()}-${cleanName}`;
        ...
        // ✅ Gerar URL PÚBLICA permanente (não temporária!)
        const { data } = supabase.storage
          .from("imoveis")
          .getPublicUrl(filePath);

        urls.push(data.publicUrl); // Esta URL é acessível publicamente
        uploaded++;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  maxLength={3}
                  required
                  value={dados.dddWhatsapp}
                  onChange={(e) => setDados({ ...dados, dddWhatsapp: e.target.value })}
                  className="w-20 border px-3 py-2 rounded text-sm"
                />
                <input
                  type="text"
                  ...
                  {dados.imagens.map((img, i) => (
                    <div key={i} className="relative">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 bg-red-600 text-white text-xs px-2 py-1 rounded-full shadow-md"
                      >
                        ✕
                      </button>
                    </div>
