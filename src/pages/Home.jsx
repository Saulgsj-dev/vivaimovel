...
const fallbackImage = "data:image/svg+xml;base64,";
...
export default function Home() {
  ...
  useEffect(() => {
    const carregarImoveis = async () => {
      try {
        ...
        alert("Erro ao carregar imóveis. Tente novamente mais tarde.");
      }
    };
    carregarImoveis();
  }, []);

  // Função para ver contato do imóvel
  const verContato = async (imovel) => {
    ...
    const jaViu = (dados.contatosVistos || []).some(
      (contato) => contato.id === imovel.id
    );
    ...
    const confirmar = window.confirm(
      "Será descontado 1 crédito para ver este contato. Deseja continuar?"
    );
    if (!confirmar) return;
      ...
      >
        <div className="bg-black/60 py-16 px-4 text-center backdrop-blur-sm rounded-xl max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-extrabold mb-2">
            Quer vender seu imóvel com agilidade?
          </h2>
          <h2 className="text-3xl md:text-5xl font-extrabold mb-2">
            Está buscando o lar ideal pra você?
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {imoveis
              ...
              .map((imovel) => (
                <div
                  key={imovel.id}
                  className="border rounded-lg shadow-sm p-4 bg-white hover:shadow-md transition flex flex-col"
                >
                  {/* Exibição da imagem */}
                  <img
                    src={
                      imovel.imagens?.[0] && isValidUrl(imovel.imagens[0])
                        ? imovel.imagens[0]
                        : fallbackImage
                    }
                    alt={imovel.titulo}
