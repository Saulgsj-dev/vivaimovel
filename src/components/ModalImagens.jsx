...
export default function ModalImagens({ imagens = [], imovel = {}, fechar }) {
  const {
    tipo, titulo, valor, area,
    quartos, suites, banheiros, vagas,
    descricao, rua, numero, cidade, bairro,
    complemento, cep
  } = imovel;
      ...
      <div className="bg-white max-w-4xl w-full rounded-lg shadow-lg overflow-y-auto max-h-[90vh] relative">
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm sm:text-base">
            ...
            <p className="sm:col-span-2">
              <span className="font-semibold">Endereço:</span> {rua}, {numero} {complemento && `- ${complemento}`} - {bairro}, {cidade} - {cep}
            </p>
          </div>
        </div>
      </div>
